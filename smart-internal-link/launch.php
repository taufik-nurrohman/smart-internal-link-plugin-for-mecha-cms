    ...
    return preg_replace_callback('#(?<!`)\{\{(article|page)\.link\:([a-z0-9\-]+)([\#?].*?)?\}\}(([^\{\}]*?)\{\{\/\1\}\})?(?!`)#s', function($matches) use($config, $speak) {
        ...
        if($data === false) {
            ...
            if($config->page_type !== 'manager') {
                File::write(sprintf($speak->plugin_smart_internal_link_title_broken_log, $config->url_current, $config->url . '/' . ($article ? $config->index->slug . '/' : "") . $matches[2]))->saveTo(PLUGIN . DS . File::B(__DIR__) . DS . 'log' . DS . $matches[1] . 's' . DS . $matches[2] . '.' . md5($config->url_current) . '.log', 0600);
            }
